...
const glossaryGroups = {
  能源: [
    ...
    ["容量因子", "发电资产实际发电量与理论最大发电量的比值，影响电力资产稳定性判断。"],
    ["并网队列", "新电源和大负荷接入电网前的审批与排队流程，决定项目落地速度。"],
    ["负荷曲线", "用电需求随时间变化的曲线，AI 数据中心会抬高基荷和峰值负荷。"],
    ["备用容量", "为应对突发停机和需求峰值预留的电力能力，负荷上行时价值提升。"],
    ["燃机调峰", "燃气机组快速启停平衡电网波动，适合配合新能源和数据中心负荷。"],
    ["需求响应", "通过价格或合约引导用电侧削峰填谷，缓解电网压力。"],
    ["储能", "把低谷电或可再生能源存起来，在高峰释放，提升数据中心供电弹性。"],
    ["N+1 冗余", "关键供电或冷却设备多配置一套备份，保障高价值算力连续运行。"],
  ],
  芯片: [
    ...
    ["CUDA", "NVIDIA GPU 软件生态，决定开发者迁移成本和算力平台黏性。"],
    ["NVLink", "GPU 之间的高速互联技术，提升多卡训练和推理吞吐。"],
    ["SerDes", "高速串并转换接口，是芯片、交换芯片和光通信互联的关键 IP。"],
    ["片上网络", "芯片内部连接计算单元和存储单元的通信结构，影响大芯片效率。"],
    ["Reticle Limit", "光刻单次曝光面积上限，限制单颗超大芯片尺寸。"],
    ["良率", "合格芯片数量占比，先进制程和先进封装越复杂，良率越关键。"],
    ["UCIe", "Chiplet 间通用互联标准，有助于不同芯粒组合成系统级芯片。"],
    ["封装基板", "承载芯片和连接外部电路的高端材料，AI 封装复杂度提升后更重要。"],
  ],
  存储: [
    ...
    ["CXL", "CPU、内存和加速器之间的高速互联协议，可能改善内存池化和资源利用率。"],
    ["对象存储", "适合海量非结构化数据的存储方式，云上训练和数据湖常用。"],
    ["冷存储", "低访问频率数据的低成本存储，适合归档和长期训练数据保留。"],
    ["QLC NAND", "单位容量成本更低的 NAND 类型，适合大容量读取密集场景。"],
    ["数据分层", "按访问频率和价值把数据放到不同存储层，优化成本和性能。"],
    ["缓存层", "把热点数据放在更快介质中，降低训练和推理数据访问延迟。"],
    ["存储带宽", "单位时间可读写数据量，影响 GPU 等待数据的时间。"],
  ],
  基础设施: [
    ...
    ["RoCE", "在以太网上实现 RDMA 的方案，用于降低 AI 集群通信延迟。"],
    ["Leaf-Spine", "数据中心常见网络拓扑，便于横向扩展和降低东西向流量瓶颈。"],
    ["CDU", "液冷冷量分配单元，连接数据中心冷源和服务器冷板。"],
    ["机柜功率密度", "单个机柜可承载的功率，AI 服务器会显著抬高该指标。"],
    ["白盒交换机", "硬件和软件解耦的网络设备，云厂商可降低成本并增强可控性。"],
    ["Kubernetes", "容器编排平台，是云原生 AI 服务部署和扩缩容的重要底座。"],
    ["Slurm", "HPC 和训练集群常见调度系统，用于分配 GPU 和作业队列。"],
  ],
  模型: [
    ...
    ["MoE", "混合专家模型，只激活部分专家网络，在扩大参数量同时控制计算成本。"],
    ["蒸馏", "用大模型训练小模型，让小模型获得更好的效果和更低推理成本。"],
    ["量化", "降低模型权重或激活精度，减少显存占用并提高推理速度。"],
    ["上下文窗口", "模型一次能处理的输入长度，影响长文档、代码和 Agent 场景。"],
    ["Embedding", "把文本、图片等转为向量表示，是检索、推荐和 RAG 的基础。"],
    ["模型路由", "按任务复杂度选择不同模型，平衡效果、成本和延迟。"],
    ["Guardrails", "模型安全护栏，用于限制输出、保护数据和降低合规风险。"],
  ],
  应用: [
    ...
    ["RPA", "机器人流程自动化，AI Agent 会把传统流程自动化升级为更灵活的执行系统。"],
    ["ITSM", "IT 服务管理，AI 可用于工单分流、故障诊断和自动修复。"],
    ["CRM Copilot", "面向销售、客服和营销流程的 AI 助手，重点看真实工作流嵌入度。"],
    ["代码助手", "辅助编程、测试和代码审查的 AI 应用，是企业生产力落地较早的方向。"],
    ["AI 搜索", "把搜索、问答和生成结合，重塑广告、知识管理和企业检索入口。"],
    ["多模态", "同时处理文本、图像、音频、视频或传感器数据的 AI 能力。"],
    ["数字孪生", "用模型模拟工厂、城市或设备运行状态，服务工业和能源优化。"],
    ["具身智能", "让机器人或自动驾驶系统在物理世界感知、决策和行动。"],
  ],
};
